...
import 'package:hayumi/oleo1.dart';
import 'package:hayumi/oleo2.dart';
import 'package:hayumi/oleo3.dart';
import 'package:hayumi/oleo4.dart';
import 'package:hayumi/oleo5.dart';
import 'package:hayumi/pneus.dart';

import 'package:hayumi/todos.dart';
import 'package:hayumi/rodas.dart';

class PagOleos extends StatefulWidget {
  const PagOleos({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _PagOleosState createState() => _PagOleosState();
}

class _PagOleosState extends State<PagOleos> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Óleos',style: TextStyle(
      ...
      body: Padding(
        ...
        child: GridView.count(
          ...
          children: [
            ImageButton(
              imagePath: 'img/oleo1.jpg',
              label: 'Óleo de Motor Lubrax 500ml',
              labelStyle: const TextStyle(color: Color.fromARGB(255, 0, 36, 84),
              fontWeight: FontWeight.bold),
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => const Oleo1()),
                );
              },
            ),
            ImageButton(
              imagePath: 'img/oleo2.jpg',
              label: 'Óleo de Motor Super SL GT-0IL 1L',
              labelStyle: const TextStyle(color: Color.fromARGB(255, 0, 36, 84),
              fontWeight: FontWeight.bold),
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => const Oleo2()),
                );
              },
            ),
            ImageButton(
              imagePath: 'img/oleo3.jpg',
              label: 'Óleo de Motor Ipiranga SI 1L',
              labelStyle: const TextStyle(color: Color.fromARGB(255, 0, 36, 84),
              fontWeight: FontWeight.bold),
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => const Oleo3()),
                );
              },
            ),
            ImageButton(
              imagePath: 'img/oleo4.jpg',
              label: 'Óleo de Motor Havoline 1L',
              labelStyle: const TextStyle(color: Color.fromARGB(255, 0, 36, 84),
              fontWeight: FontWeight.bold),
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => const Oleo4()),
                );
              },
            ),
            // Botão com imagem
            ImageButton(
              imagePath: 'img/oleo5.jpg',
              label: 'Óleo de Motor Mobil 20w50 1L',
              labelStyle: const TextStyle(color: Color.fromARGB(255, 0, 36, 84),
              fontWeight: FontWeight.bold),
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => const Oleo5()),
                );
              },
            ),
          ],
